...
public class WorldService {
	...
	public List<Country> getAllCountries() {
		CountryDAO cd = new CountryPostgresDaoImpl();
		return cd.findAll();
	}
	
	public List<Country> get10LargestPopulations() {
		CountryDAO cd = new CountryPostgresDaoImpl();
		return cd.find10largestPopulations();
	}

	public List<Country> get10LargestSurfaces() {
		CountryDAO cd = new CountryPostgresDaoImpl();
		return cd.find10largestSurfaces();
	}
	
	public Country getCountryByCode(String code) {
		CountryDAO cd = new CountryPostgresDaoImpl();
		return cd.findByCode(code);
	}
	public boolean save(Country country){
        CountryDAO cd = new CountryPostgresDaoImpl();
        return cd.save(country);
    }
	public boolean delete(Country country){
		CountryDAO cd = new CountryPostgresDaoImpl();
		return cd.delete(country);
	}
	public boolean update(Country country){
		CountryDAO cd = new CountryPostgresDaoImpl();
		boolean succes = cd.update(country);
		System.out.println(
				succes
		);
		return succes;
